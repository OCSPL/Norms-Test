...
from Products_Files.Calculate_24dcbc import Calculate_24dcbc,calculate_24dc
...
from Products_Files.calculate_tbbc import calculate_tbbc
...
from Products_Files.calculate_mdea import calculate_mdea,calculate_con_mdea
from Products_Files.calculate_mmea import calculate_mmea,calculate_con_mmea
from Products_Files.calculate_c5 import calculate_c5
...
# ─────────────────────────────────────────────────────────────────────────────
def _batch_range_for_item(cons_df, item_name, col='Consume_Batch_No'):
    """
    From the filtered consumption dataframe pick rows for *item_name*, collect
    distinct batch numbers found in *col*, sort by the last 4 digits, and
    return (batch_from, batch_to, total_batches).  If none → ("", "", 0)
    """
    if col not in cons_df.columns:
        return "", "", 0

    batches = (
        cons_df.loc[cons_df['Consume_Item_Name'] == item_name, col]
        .dropna()
        .astype(str)
        .str.strip()
        .unique()
    ...
    batches_sorted = sorted(
        batches,
        key=lambda b: int(b[-4:]) if b[-4:].isdigit() else 0
    )
    return batches_sorted[0], batches_sorted[-1], len(batches_sorted)
# ─────────────────────────────────────────────────────────────────────────────
...
def process_data(start_date, end_date, fg_name):
    ...
    # job_work_df.to_csv('job_work_df.csv', index=False, encoding='utf-8-sig')    
    ...
    if start_date and end_date:
        ...
        if fg_name in fg_name_to_items:
            ...
            # ── Stage-wise batch range (ONLY from WIP CONSUMED IN PRODUCTION) ────────────
            batch_from_list, batch_to_list, batch_cnt_list = [], [], []
            for wip_item in stock_summary['Item Name']:
                bf, bt, cnt = _batch_range_for_item(filtered_consumption, wip_item)
                batch_from_list.append(bf)
                batch_to_list.append(bt)
                batch_cnt_list.append(cnt)

            stock_summary['Batch_From']    = batch_from_list
            stock_summary['Batch_To']      = batch_to_list
            stock_summary['Total_Batches'] = batch_cnt_list
# ─────────────────────────────────────────────────────────────────────────────
            ...
            stock_summary = stock_summary[['Item Name', 'Stage', 'Opening', 'WIP PRODUCED', 'WIP CONSUMED IN PRODUCTION',
                                           ...
                                           'Quantities_Consumed_mult', 'Additional_QTY_mult', 'NET QTY','Batch_From', 'Batch_To', 'Total_Batches', ]]
        ...
        if fg_name == '2,3 DI CHLORO PYRIDINE':
        ...
        elif fg_name == '2,4 DICHLORO BENZYL CHLORIDE':
            stock_summary, bom_summaries_df = calculate_24dc(stock_summary, bom_summaries_df)
            ...
            stock_summary, bom_summaries_df = calculate_mdea(stock_summary, bom_summaries_df)
        elif fg_name == 'M-MEA':
            stock_summary, bom_summaries_df = calculate_mmea(stock_summary, bom_summaries_df)
        ...
        elif fg_name == '4-TERTIARY BUTYL BENZYL CYANIDE':
            stock_summary = calculate_tbbc(stock_summary, bom_summaries_df)
        ...
        if fg_name:
            ...
            # Case 2: 2,4 DICHLORO BENZYL CHLORIDE → add Output_Quantity where Output_Item_Name == '2,4 DCBC SFG'
            if fg_name == '2,4 DICHLORO BENZYL CHLORIDE':
                add_qty = (
                    pd.to_numeric(
                        df_job_work2_filtered.loc[
                            df_job_work2_filtered['Output_Item_Name'].astype(str).str.strip() == '2,4 DCBC SFG',
                            'Output_Quantity'
                        ],
                        errors='coerce'
                    ).fillna(0).sum()
                ...
                output_quantity=output_quantity_df['Output_Quantity'].sum() + add_qty
            elif fg_name == '4-HYDROXY-3-(2,4,6-TRIMETHYLPHENYL)-1-OXASPIRO[4.4]NON-3-EN-2-ONE':
                add_qty = (
                    pd.to_numeric(
                        df_job_work2_filtered.loc[
                            df_job_work2_filtered['Output_Item_Name'].astype(str).str.strip() == '4-HYDROXY-3-(2,4,6-TRIMETHYLPHENYL)-1-OXASPIRO[4.4]NON-3-EN-2-ONE',
                            'Output_Quantity'
                        ],
                        errors='coerce'
                    ).fillna(0).sum()
            ...
            # NEW ──────────────────────────────────────────────────────────────
            batches_sorted = sorted(
                output_quantity_df['Output_Batch_No'].unique(),      # deduplicate
                key=lambda b: int(b[-4:])                            # sort by last 4 digits
            )

            Total_batch = len(batches_sorted)
            batch_from  = batches_sorted[0] if batches_sorted else ""
            batch_to    = batches_sorted[-1] if batches_sorted else ""
            batch_range = f"{batch_from} – {batch_to}"   
        ...
        if fg_name == 'AMIDO CHLORIDE':
            ...
            net_qty_amido_recovered1 = stock_summary.loc[stock_summary['Item Name'] == 'AMIDO IPAC MLR', 'NET QTY'].values[0]
            Con_qty.loc[Con_qty['Consume_Item_Name'] == 'ISO PROPYL ACETATE (M)', 'WIP-RM'] = net_qty_amido_recovered + net_qty_amido_recovered1 
        elif fg_name == '2,4,6 TRIMETHYL PHENYL ACETYL CHLORIDE':
            ...
            Con_qty = calculate_con_mdea(Con_qty, stock_summary)
        elif fg_name == 'M-MEA':
            Con_qty = calculate_con_mmea(Con_qty, stock_summary)
        elif fg_name == 'TPA':
            ...
            Con_qty = calculate_sipro_con(Con_qty, stock_summary,job_work_df_filtered)
        elif fg_name == 'DMA-CHLORIDE LAN':
            Con_qty = calculate_ocdb(Con_qty, stock_summary)
        elif fg_name == 'C-5 HYDROXY ESTER':
            Con_qty = calculate_c5(Con_qty, job_work_df_filtered)
        ...
        # ── GLOBAL WIP-CONSUMPTION BATCH RANGE ──────────────────────────────────
        if 'Consume_Batch_No' in filtered_consumption.columns:
            wip_batches = (
                filtered_consumption['Consume_Batch_No']
                .dropna()
                .astype(str)
                .str.strip()
                .unique()
            ...
            if wip_batches:
                wip_batches_sorted = sorted(
                    wip_batches,
                    key=lambda b: int(b[-4:]) if b[-4:].isdigit() else 0
                )
                wip_batch_from  = wip_batches_sorted[0]
                wip_batch_to    = wip_batches_sorted[-1]
                wip_total_batch = len(wip_batches_sorted)
            else:
                wip_batch_from = wip_batch_to = ""
                wip_total_batch = 0
        else:
            wip_batch_from = wip_batch_to = ""
            wip_total_batch = 0
        wip_batch_range = f"{wip_batch_from} – {wip_batch_to}" if wip_batch_from else ""
        # ────────────────────────────────────────────────────────────────────────
        ...
        data = {
            ...
            # ── Batch information ───────────────────────────────────────────
            'batch_from':        batch_from,          # e.g. 26ATMPA4020001
            'batch_to':          batch_to,            # e.g. 26ATMPA4020013
            'total_batches':     Total_batch,         # integer count
            'batch_range':       f"{batch_from} – {batch_to}",  # handy pre-formatted string
            # ── NEW global WIP-consumption batch info ──
            'wip_batch_from':     wip_batch_from,
            'wip_batch_to':       wip_batch_to,
            'wip_total_batches':  wip_total_batch,
            'wip_batch_range':    wip_batch_range,
