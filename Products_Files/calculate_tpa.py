...
def calculate_tpa(stock_summary, bom_summaries_df):
    ...
    try:
        additional_qty_for_pick1 = bom_summaries_df.loc[
            (bom_summaries_df['Stage Name'] == 'TFAC STAGE-II CRUDE') &
            (bom_summaries_df['Name'] == 'TFAC STAGE-I'),
            'RM WIP QTY'
        ].iloc[0]
    except IndexError:
    ...
    stock_summary.loc[
        stock_summary['Item Name'] == 'TFAC STAGE-I CRUDE',
    ...
    bom_summaries_df = update_bom_summaries_with_net_qty(stock_summary, bom_summaries_df, 'TFAC STAGE-I CRUDE')
    
    return stock_summary, bom_summaries_df


def calculate_con_tpa(Con_qty,stock_summary):
    # Filter the stock_summary DataFrame for '2,4,6 RECOVERED MESITYLENE'
    mesitylene_row = stock_summary[stock_summary['Item Name'] == 'TFAC RECOVERED BENZOTRIFLURIDE']
    ...
    mesitylene_con_qty_row = Con_qty[Con_qty['Consume_Item_Name'] == 'BENZOTRIFLUORIDE (M)']
    ...
    Con_qty = Con_qty.loc[Con_qty['Consume_Item_Name'].astype(str).str.strip().str.upper() != 'TPA (M)'].copy()
