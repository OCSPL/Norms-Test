...
def get_sys_bipro(from_date, to_date, fg_name):
    return text('''
      -- Add Aliases for clarity when joining FG tables
SELECT           
    DET_BP.lLnkDocId              AS FG_Production_Lid, -- Linked FG Production Header ID
    HDR_BP.lid                    AS BiProduct_lid,
    HDR_BP.sDocNo                 AS [BiProduct_Voucher_No],
    CONVERT(varchar, CONVERT(date, CONVERT(varchar(8), HDR_BP.dtDocDate), 112), 106) AS [BiProduct_Voucher_Date], -- Using 106 for DD Mon YYYY format
    ...
    -- >>> Added FG Information <<<
    ISNULL(CF_FG.sValue, '')      AS [FG_Name],          -- FG Name from ITMCF (Field 10)
    DET_FG.sValue1                AS [Output_Batch_No],  -- FG Batch from FG Output Line (sValue1)
    ITM_FG.sName                  AS [FG_Item_Name],     -- Standard FG Item Name
    HDR_FG.sDocNo                 AS [FG_Voucher_No]     -- FG Production Voucher No

FROM txnhdr AS HDR_BP -- BiProduct Transaction Header

INNER JOIN TXNDET AS DET_BP -- BiProduct Transaction Detail
    ON HDR_BP.lId = DET_BP.lId AND DET_BP.cFlag = 'I' 

INNER JOIN ITMMST AS ITM_BP -- BiProduct Item Master
    ON DET_BP.lItmId = ITM_BP.lId

INNER JOIN UNTMST AS UOM_BP -- BiProduct UOM Master
    ON DET_BP.lUntId = UOM_BP.lId

-- Join to the Parent FG Production Transaction using the Link ID
INNER JOIN txnhdr AS HDR_FG 
    ON DET_BP.lLnkDocId = HDR_FG.lId -- Link BiProduct Detail to FG Header

-- Join to the FG Output Detail line within the FG Production Transaction
INNER JOIN TXNDET AS DET_FG 
    ON HDR_FG.lId = DET_FG.lId 
   AND DET_FG.cFlag = 'I' -- Assuming 'I' identifies the main FG Output line

-- Join to the FG Item Master
INNER JOIN ITMMST AS ITM_FG 
    ON DET_FG.lItmId = ITM_FG.lId

-- Join to the FG Custom Fields table to get the FG Name (Assuming FieldNo 10)
LEFT JOIN ITMCF AS CF_FG 
    ON DET_FG.lItmId = CF_FG.lId 
   AND CF_FG.lFieldNo = 10 -- *** Adjust FieldNo if FG_Name is stored differently ***
   AND CF_FG.lLine = 0     -- *** Adjust lLine if needed ***

-- Removed join to ITMTYP as it wasn't used in the SELECT list for BiProduct
-- INNER JOIN ITMTYP AS ITP ON ITP.lTypid = ITM_BP.lTypid -- Corrected join key assumption if needed

WHERE 
    -- Filter for BiProduct Transaction Types
    HDR_BP.ltypid IN (599,893,912,915,928,929,1059,1060,1061,1062,1063,1064)           
    AND HDR_BP.lcompid = 27
    AND HDR_BP.bDel = 0
    
    -- Add Filters for the Parent FG Production Transaction as well (Good Practice)
    AND HDR_FG.ltypid IN (597, 924, 913, 925, 899, 891) -- Add known FG Production Types
    AND HDR_FG.bDel = 0
	and CF_FG.sValue = :fg_name
	and HDR_BP.dtDocDate BETWEEN :from_date AND :to_date

ORDER BY 
    [FG_Voucher_No], [BiProduct_Item_Name]; -- Example ordering
        ''')
